...
#
#      VERIFIED AGAINST THE ACTUAL REPO CONTENT (2026-08-12, in response to
#      a direct question about whether the attacker truly controls this
#      build target's content):
#        - `examples/MODULE.bazel` declares `module(name = "rules_cuda_examples", ...)`,
#          so `@rules_cuda_examples` is a SELF-REFERENTIAL bzlmod label --
...
#          `@nccl-tests//:*_perf` targets. `@nccl`/`@nccl-tests` themselves
#          ARE fetched via `http_archive` with pinned `sha256`+`urls` in
#          `examples/WORKSPACE.bzlmod` -- an attacker cannot tamper with
#          the fetched upstream NCCL source that way.
#        - However, `examples/nccl/BUILD.bazel` itself -- the file that
#          defines `perf_binaries` and wires in its dependencies -- is a
#          completely ordinary file inside the attacker's own fork PR.
#          An attacker does not need to defeat the sha256 pinning at all:
#          they can simply add a new target to this same file and make
#          `perf_binaries` (transitively) depend on it, exactly as this
#          PoC's `attacker_fork_pr/examples/nccl/BUILD.bazel` does.
...
#      tree as part of resolving that build.
...
#
# THIS SCRIPT'S QUESTION: does a malicious addition to
# examples/nccl/BUILD.bazel -- the REAL file defining the REAL
# `perf_binaries` target the real workflow builds -- actually execute as
# part of `bazelisk build` on that target?
#
...
#     PR's checked-out HEAD would contain, laid out to match the real
#     repo's path (`examples/nccl/BUILD.bazel`), with a genrule added
#     alongside the real `perf_binaries` target definition, modeling the
#     "attacker-controlled build action" step,
...
echo "    modifies examples/nccl/BUILD.bazel (the REAL file that defines"
echo "    the REAL perf_binaries target -- this stands in for what the"
echo "    real 'Checkout PR branch' step in test-comment would pull in):"
echo "-----------------------------------------------------"
cat examples/nccl/BUILD.bazel
echo "-----------------------------------------------------"
echo ""
...
echo "    would, against the exact target path (examples/nccl:perf_binaries)"
echo "    the real workflow builds:"
echo ""

MOCK_GITHUB_TOKEN="ghs_MOCKTOKEN_do_not_use_1234567890abcdef" \
  bazelisk build --verbose_failures //examples/nccl:perf_binaries 2>&1 | tee bazel_build_output.log
